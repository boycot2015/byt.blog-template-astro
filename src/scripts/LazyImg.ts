...
export default () => {
  document.querySelectorAll(".main-inner>.main-inner-content img:not(.view-image-container)").forEach((i: any) => {
    // 是否包含data-byt-lz-src
    if (!i.hasAttribute("data-byt-lz-src")) {
      i.setAttribute("data-byt-lz-src", i.getAttribute("src"));
      i.setAttribute("src", '/assets/images/lazy-loading.webp');
    }
  });
  if (lazyLoadStatus) return lazyLoadStatus.update();
  lazyLoadStatus = new LazyLoad({ elements_selector: "img:not(.view-image-container)", threshold: 0, data_src: "byt-lz-src" });
}